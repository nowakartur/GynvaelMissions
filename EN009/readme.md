...
I have marked 3 blocks that seems interesting:


![binary file in IDA Pro - 03](https://github.com/nowakartur/GynvaelMissions/raw/master/EN009/img/ida_03.png)

**Block a)** - this is first usage of function time64() [description](http://www.cplusplus.com/reference/ctime/time/) - is there some time-dependent steps ??

**Block b)** - initialization od random numer generator with value received in previous block - Time function returns [unix timestamp](https://en.wikipedia.org/wiki/Unix_time)
...
**Block d)** - this is only place where XOR operation is used so it must be it :). Function **_mm_xor_si128** is SSE2 powered XOR operation: [msdn desc](goo.gl/5oxH5a)


Conclusion:

Agent used randomly generated one-time-pad for XOR message with key. Random number generator was initialized with value of current timestamp (script execution time)
...
Problem with timestamp: I think there i no information about compilation time in object file (I didn't find any), so I have to bruteforce it...  I assumed that message was created maxixmum one week before I got it. I will have to check 7 (days) * 24 (hours) * 60 (minutes) * 60 (seconds) timestapmps (604800!). I think it is doable :)
...
But for 100 % sure I will generate random numbers by **GCC: (x86_64-win32-seh-rev0, Built by MinGW-W64 project) 7.1.0**.


**Scripts**

Message I need to decrypt has 31 bytes, so I will generate 32 long random numbers for every timestamp begining moment when i received task, and week ago.
...
There was a suprise, file size generated by this script, executed with redirected output fo file, was over 110 MB :) Lots of random numbers :)


Python script I use for searching correct timestamp, key and finally decrypted message:
...
As I said before I must replicate this strange meth calculations:
