...
I think this is a message from author (this message overwrites secret I need to find):
```
...
Problem with timestamp: I think there is no information about compilation time in object file (I didn't find any), so I have to bruteforce it...  I assumed that message was created maximum one week before I got it. I will have to check 7 (days) * 24 (hours) * 60 (minutes) * 60 (seconds) timestamps (604800!). I think it is doable :)

Problem with rand() implementations: After my short research i found there is many rand() implementations, even in C language there is huge differences (for example between glibc and MS Visual distribution): [wikipedia link](https://en.wikipedia.org/wiki/Linear_congruential_generator)
...
Message I need to decrypt has 31 bytes, so I will generate 32 long random numbers for every timestamp from moment when I received task and ending week earlier.
...
There was a surprise, file size generated by this script (script was executed with redirected output fo file) was over 110 MB :) Lots of random numbers :)
